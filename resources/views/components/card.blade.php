@props(['blog'])
<div class="flex flex-col bg-yellow-300 px-5 py-10 space-y-8 rounded-lg">
    ...
    <h2 class="text-xl flex-1">
        Author - <span class="font-bold">{{$blog->author->name}}</span>
    </h2>
    <div class="cursor-pointer flex-1">
        <a class="bg-slate-900 text-yellow-400 px-2 py-1 rounded-lg" href={{"blogs/category/".$blog->category->slug}}>
            {{$blog->category->name}}
        </a>
    </div>
